...
class BeanDefinitionLoader {
	...
	BeanDefinitionLoader(BeanDefinitionRegistry registry, Object... sources) {
		...
		// 注解形式的bean定义读取器（作用：读取注解标注的bean），比如：@Configuration、@Bean、@Component、@Service等待
	...
	private int load(Class<?> source) {
		...
		// 检查source（主启动类）上面有没有添加@Component注解
		if (isComponent(source)) {
			// 通过annotatedReader读取器，将 主启动类的beanDefinition，注册到beanDefinitionMap中

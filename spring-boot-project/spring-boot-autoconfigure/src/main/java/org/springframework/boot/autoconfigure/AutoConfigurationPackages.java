...
public abstract class AutoConfigurationPackages {
	...
	public static void register(BeanDefinitionRegistry registry, String... packageNames) {
		// 这里参数packageNames缺省的情况下就是一个字符串，是使用了注解@SpringBootApplication的启动类所在的包
		if (registry.containsBeanDefinition(BEAN)) {
			...
			// 如果该bean没有注册，则注册该bean，参数中提供的包名称会被设置到bean定义中去
			GenericBeanDefinition beanDefinition = new GenericBeanDefinition();
			beanDefinition.setBeanClass(BasePackages.class);
			beanDefinition.getConstructorArgumentValues().addIndexedArgumentValue(0, packageNames);
			beanDefinition.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
			// 可以看到这里注册了 AutoConfigurationPackages 的定义信息到容器，后续会根据bean定义信息被实例化
	...
	static class Registrar implements ImportBeanDefinitionRegistrar, DeterminableImports {
		...
		public void registerBeanDefinitions(AnnotationMetadata metadata, BeanDefinitionRegistry registry) {
			// 注册一些bean的定义信息到容器
			// 将注解标注的源信息传入，获取到对应的包名（也就是被AutoConfigurationPackage标注的类所在的包（说的直白一点就是主启动类所在的包））
